...
\textbf{RecruitSense} is proposed as a comprehensive, decoupled web-based Applicant Tracking System and Intelligent Resume Screening Platform engineered to streamline recruitment workflows, automate candidate screening, and deliver bidirectional transparency. The platform is architected with a responsive Single Page Application (SPA) frontend built using React 18, Vite, and Tailwind CSS, communicating with a high-throughput RESTful API service engineered with Laravel 11 and a relational MySQL database. Computational intelligence is powered by a dedicated Python Flask microservice utilizing Natural Language Processing (NLP) for PDF document parsing, boundary-delimited skill matching, and TF-IDF cosine similarity scoring \cite{salton1988term, sivaram2022nlp}.
...
\begin{itemize}
    ...
    \item \textbf{Transparent Candidate Feedback and Skill Gap Diagnostics:} Automatically displays matched technical skills, missing required skills, and overall ATS compatibility directly to job seekers upon application.
    \item \textbf{Multi-Tenant Role-Based Workspaces:} Provides dedicated, tailored web portals for Job Seekers (application tracking, job discovery), Corporate Recruiters (job posting, candidate shortlists, pipeline management), and Administrators (domain verification, platform auditing).
    ...
    \item \textbf{Real-Time In-App Messaging and Interview Scheduling:} Facilitates direct recruiter-candidate coordination and structured interview scheduling within the platform.
\end{itemize}
...
\textbf{FR-01} & \textbf{User Registration and Authentication:} Allow users to register as Job Seekers, Recruiters, or Admins and authenticate securely via Laravel Sanctum bearer tokens. \\ \hline
\textbf{FR-02} & \textbf{Profile Management:} Enable Job Seekers and Employers to maintain profiles, upload profile images, manage bios, and configure account settings. \\ \hline
...
\textbf{FR-07} & \textbf{AI PDF Text Extraction:} Ingest uploaded resume documents and extract sanitized UTF-8 text streams page-by-page using the Flask AI microservice. \\ \hline
...
\textbf{NFR-01} & \textbf{Performance and Latency:} Ensure standard REST API responses occur within $300\text{ ms}$, with end-to-end AI resume parsing and ATS scoring executing under $2.5\text{ seconds}$. \\ \hline
...
\textbf{NFR-05} & \textbf{High Usability and Responsive UI:} Provide an intuitive, modern responsive interface built with Tailwind CSS supporting desktop and laptop displays. \\ \hline
...
\textbf{NFR-07} & \textbf{Horizontal Scalability:} Design the React frontend, Laravel API backend, and Flask AI service as decoupled tiers capable of independent scaling. \\ \hline
...
\textbf{NFR-09} & \textbf{Maintainability and Modular Code:} Adhere to standard MVC design patterns in Laravel and component-driven architecture in React. \\ \hline
