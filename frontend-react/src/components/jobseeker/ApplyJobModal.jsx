...
import { Banknote, Briefcase, Loader2, MapPin, Send, Sparkles, Timer, X } from 'lucide-react'
    ...
    <div className="fixed inset-0 z-50 bg-gray-900/40 flex items-center justify-center px-4">
      <form onSubmit={submitApplication} className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-5 overflow-y-auto space-y-5">
          <div className="rounded-xl border border-indigo-100 bg-gradient-to-r from-indigo-50 to-purple-50 p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="text-sm font-bold text-indigo-900">Instant AI Resume Analysis & Quiz Readiness</p>
                <p className="text-xs text-indigo-700 mt-1">
                  RecruitSense AI will score your resume match, check skill gaps, and prepare a 5-minute skill assessment for this role.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 bg-white flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          {loading ? (
            <div className="flex items-center gap-2 text-xs font-semibold text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-lg">
              <Sparkles className="w-3.5 h-3.5 text-indigo-500 animate-spin" />
              <span>Analyzing resume & preparing assessment...</span>
            </div>
          ) : (
            <div className="hidden sm:block text-xs text-gray-400">
              Instant AI evaluation on submit
            </div>
          )}
